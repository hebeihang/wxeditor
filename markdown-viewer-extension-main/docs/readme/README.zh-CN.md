...
**以前：** 复制内容 → 设置格式 → 调列表 → 加背景 → Excel 制图截图 = **每周 1 小时**
...
**Mermaid 流程图** · **Vega/Vega-Lite 数据图表** · SVG 图片 · 复杂 HTML 表格
...
- 传统工具：画图 30 分钟 + 修改 20 分钟 + 调整 10 分钟 + 导出 5 分钟 = **65 分钟**
- Markdown Viewer：写代码 5 分钟 + 修改 30 秒 + 导出 1 秒 = **6 分钟**

**商务场景：** 季度销售报表（5 个柱状图）

- Excel 制图 + 截图：选数据 15 分钟 + 调格式 10 分钟 + 截图保存 5 分钟 = **30 分钟**
- Vega-Lite：JSON 数据 2 分钟 + 一键导出 = **3 分钟**
...
- **第一次打开：** 文字立即显示，图表后台渲染，5 秒内全部完成
- **第二次打开：** 从缓存读取，瞬间显示（<1 秒）
- **修改文字：** 还是秒开（图表从缓存读取）
- **修改图表：** 只重新渲染修改的图表
...
- **三种布局：** 正常（1000px）/ 满屏 / 窄屏（530px，预览 Word 效果）
...
**Q: 支持哪些图表？**  
A: 所有 Mermaid 图表（流程图、时序图、甘特图、类图、状态图、饼图、ER 图等）、Vega / Vega-Lite 数据可视化图表 + SVG 自动转换。
