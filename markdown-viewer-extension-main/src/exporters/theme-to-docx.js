...
function generateDefaultStyle(fontScheme, spacingScheme) {
  ...
  const baseFontSizePt = Number.parseFloat(fontScheme.body.fontSize)
...
function generateParagraphStyles(fontScheme, spacingScheme) {
  ...
  Object.keys(fontScheme.headings).forEach((level, index) => {
    ...
    const headingBeforePt = heading.spacing?.before ? Number.parseFloat(heading.spacing.before) : 0
    const headingAfterPt = heading.spacing?.after ? Number.parseFloat(heading.spacing.after) : 0
...
function parseBorderWidth(width, style = 'single') {
  ...
  if (!match)
    return 8 // Default 1pt = 8 eighths
